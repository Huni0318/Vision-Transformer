...
class LinearProjection(nn.Module):
    ...
    def __init__(self, patch_vec_size, num_patches, latent_vec_dim, drop_rate):
        ...
        self.cls_token = nn.Parameter(torch.randn(1, letent_vec_dim))
        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches+1, latent_vec_dim))
        self.dropout = nn.Dropout(drop_rate)
        
        
    def forward(self, x):
        batch_size = x.size(0) #bxnxp^2xc
        x = torch.cat([self.cls_token.repeat(batch_size, 1, 1), self.linear_proj(x)], dim=1) # bxnxd
...
class MultiheadedSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        k = k.view(batch_size, -1, self.num_heads, self.head_dim).permute(0,2,3,1) #k_T
...
class TFencoderLayer(nn.Module):
    def __init__(self, latent_vec_dim, num_heads, mlp_hidden_dim, drop_rate):
        super().__init__()
        self.1n1 = nn.LayerNorm(latent_vec_dim)
...
class VisionTransformer(nn.Module):
    def __init__(self, patch_vec_size, num_patches, latent_vec_dim, num_heads, mlp_hidden_dim, drop_rate, num_layers, num_classese):
        super().__init__()
